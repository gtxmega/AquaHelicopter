...
public class Hovl_Laser2 : MonoBehaviour
{
    ...
    private void Update()
    {
        if (laserPS != null && UpdateSaver == false)
        {
            ...
            laserMat.SetVector("_StartPoint", thisTransform.position);
            //Set end laser point
            RaycastHit hit;
            if (Physics.Raycast(thisTransform.position, thisTransform.TransformDirection(Vector3.forward), out hit, MaxLength))
            {
            ...
            else
            {
                ...
                var EndPos = thisTransform.position + thisTransform.forward * MaxLength;
                var distance = Vector3.Distance(EndPos, thisTransform.position);
